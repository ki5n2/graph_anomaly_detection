...
class BertEncoder(nn.Module):
    def __init__(self, num_features, hidden_dims, nhead, num_layers, max_nodes, dropout_rate=0.1):
...
def run(dataset_name, random_seed, dataset_AN, split=None, device=device):
    ...
    recon_optimizer = torch.optim.Adam(
        list(model.feature_decoder.parameters()) + 
        list(model.edge_decoder.parameters()), 
        lr=learning_rate,
