...
def train(model, train_loader, recon_optimizer, device, epoch):
    ...
    for data in train_loader:
        ...
        train_cls_outputs, x_recon, cls_projected = model(x, edge_index, batch, num_graphs)
        ...
        # Persistence features 계산
        persistence_features = compute_persistence_features(x, edge_index, batch)
        persistence_loss = F.mse_loss(cls_projected, persistence_features)
...
def evaluate_model_with_density(model, test_loader, cluster_centers, n_clusters, gamma_clusters, random_seed, epoch, trial, reconstruction_errors, device):
    ...
    with torch.no_grad():
        for data in test_loader:
            data = data.to(device)
            x, edge_index, batch, num_graphs = data.x, data.edge_index, data.batch, data.num_graphs
            e_cls_output, x_recon, cls_projected = model(x, edge_index, batch, num_graphs)
...
def compute_persistence_features(x, edge_index, batch):
    batch_size = batch.max().item() + 1
    diagrams_list = []
    
    for i in range(batch_size):
        mask = batch == i
        nodes = x[mask].cpu().numpy()
        ...
        # 데이터가 모두 0이 아닌지 확인
        if np.all(nodes == 0):
            diagrams_list.append([0.0] * 12)
            continue
            
        # 정규화 (안전하게)
        scaler = StandardScaler(with_mean=True, with_std=True)
        try:
            nodes_normalized = scaler.fit_transform(nodes)
        except:
            nodes_normalized = nodes
            
        # 거리 행렬 계산
        dist_matrix = cdist(nodes_normalized, nodes_normalized)
        ...
        try:
            ...
            for dim in range(3):
                ...
                if len(diagram) > 0:
                    ...
                    persistence_lengths = np.nan_to_num(
                        diagram[:, 1] - diagram[:, 0], 
                        nan=0.0, posinf=0.0, neginf=0.0
                    )
                    if len(persistence_lengths) > 0:
                        features.extend([
                            float(np.mean(persistence_lengths)),
                            float(np.std(persistence_lengths)) if len(persistence_lengths) > 1 else 0.0,
                            float(np.max(persistence_lengths)),
                            float(len(persistence_lengths))
                        ])
                    else:
            ...
            diagrams_list.append(features)
            
        except Exception as e:
            print(f"Error in graph {i}: {str(e)}")
            diagrams_list.append([0.0] * 12)
            
    persistence_features = torch.tensor(diagrams_list, dtype=torch.float32)
    return persistence_features.to(x.device)
...
class GRAPH_AUTOENCODER(nn.Module):
    def __init__(self, num_features, hidden_dims, max_nodes, nhead_BERT, nhead, num_layers_BERT, num_layers, dropout_rate=0.1):
        ...
        self.persistence_projection = nn.Linear(hidden_dims[-1], 12)  # 12 = 4 features * 3 dimensions
    ...
    def forward(self, x, edge_index, batch, num_graphs, mask_indices=None, training=False, edge_training=False):
        ...
        cls_projected = self.persistence_projection(cls_output)
        ...
        return cls_output, x_recon, cls_projected
...
def run(dataset_name, random_seed, dataset_AN, trial, device=device, epoch_results=None):
    ...
    return auroc, epoch_results, explained_variance_ratio, eigenvalues
...
if __name__ == '__main__':
    ...
    for trial in range(n_cross_val):
        ...
        ad_auc, epoch_results, explained_variance_ratio, eigenvalues = run(dataset_name, random_seed, dataset_AN, trial, device=device, epoch_results=epoch_results)
